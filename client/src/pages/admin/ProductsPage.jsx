...
export default function ProductsPage() {
  ...
  return (
    <div className="admin-page">
      ...
      <section className="panel table-panel"><div className="table-scroll"><table><thead><tr><th>Sản phẩm</th><th>Hãng</th><th>Biến thể</th><th>Tồn kho</th><th>Giá từ</th><th>Trạng thái</th><th /></tr></thead><tbody>
        {products.map((product) => {
          const prices = product.variants.map((variant) => variant.salePrice);
          const totalStock = product.variants.reduce((sum, v) => sum + v.stock, 0);
          return <tr key={product._id}><td><strong>{product.name}</strong><small>{product.modelCode}</small></td><td>{product.brandId?.name || '—'}</td><td>{product.variants.length}</td><td><strong>{totalStock}</strong></td><td>{currency.format(Math.min(...prices))}</td><td><span className={product.isActive ? 'status status--active' : 'status'}>{product.isActive ? 'Đang hiện' : 'Đang ẩn'}</span></td><td className="table-actions"><Link to={`/admin/products/${product._id}/edit`}>Sửa</Link><button onClick={() => toggleStatus(product)}>{product.isActive ? 'Ẩn' : 'Hiện'}</button></td></tr>;
        })}
        {products.length === 0 && <tr><td colSpan="7" className="empty-cell">Chưa có sản phẩm. Hãy thêm sản phẩm đầu tiên.</td></tr>}
